        ...
        if (message.author.id != message.guild.ownerId) {
            if (member.roles) {
                if (!(message.member.roles.highest.position > member.roles.highest.position))
                    return await message.channel.send(':x: | You can\'t unmute the selected user');
            }
        }
        if (!member.moderatable)
            return await message.channel.send(':x: | I can\'t unmute the selected user');
        if (!member.isCommunicationDisabled())
            return await message.channel.send(':x: | The user isn\'t muted');
